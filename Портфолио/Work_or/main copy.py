...
class MainWindow(QMainWindow):
    ...
    def load_competitors_file(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Выберите файл competitors2.json", "", "JSON Файлы (*.json)")
        if file_path:
                try:
    ...
    def load_results_file(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Выберите файл results_RUN.txt", "", "Текстовые файлы (*.txt)")
        if file_path:
...
if __name__ == "__main__":
    ...
    # palette.setColor(QPalette.Window, QColor(255, 182, 193))  # установка серого цвета
