...
def f(x):
    return 2/(1 + np.exp(-x)) - 1 #Функция Гиперболического тангенса

def df(x):
    return 0.5*(1 + x)*(1 - x) #Функция вычесление производной


W1 = np.array([[-0.2, 0.3, -0.4], [0.1, -0.3, -0.4]]) #Инициализируем веса 1й слой
W2 = np.array([0.2, 0.3]) #2й слой


def go_forward(inp):  #Функция пропускает вектор наблюдений через неиронную сеть
    sum = np.dot(W1, inp)
...
def train(epoch): #Функция которая обучает неиронную сеть
    global W2, W1
